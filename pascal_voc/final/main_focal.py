...
criterion = fn.FocalLoss(alpha=0.25, gamma=2, ignore_index=255).to(DEVICE)
...
if brestore:
    ...
    best_pixel_accuracy = checkpoint.get('best_pixel_accuracy', 0.0)
...
for it in range(restore_iter, num_training + 1):
    ...
    if it % 1000 == 0 and it != 0:
        ...
        for itest in range(len(test_img)):
            ...
            correct_pixels += np.sum((pred == gt) & valid)
        ...
        if patience_counter >= patience:
            print('Early stopping / best acc: %.2f%%' % best_pixel_accuracy)
            break
...
log_file.close()
print('Training done / best acc: %.2f%%' % best_pixel_accuracy)